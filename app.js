...
const state = {
  ...
  sequences: [],
  papers: [],
  tree: null, // Will be loaded from Newick file
  activePanel: null,
  filters: {},
  descriptors: [],
  descriptorInfo: {}
 ...
 * @returns {object} A hierarchical object for D3.
 */
function parseNewick(newick) {
    ...
    function parseSubtree() {
        if (newick[pos] === '(') {
            ...
            pos++; // Skip ')'
            const node = { branchset: children };
            const { name, length } = parseNameAndLength();
            if (name) node.name = name;
            if (length) node.length = length;
            return node;
        } else {
            const { name, length } = parseNameAndLength();
            return { name: name || 'internal', length: length };
        }
    }
    ...
    function parseNameAndLength() {
        ...
        while (pos < newick.length && newick[pos] !== ',' && newick[pos] !== ')' && newick[pos] !== ';') {
            if (newick[pos] === ':') {
                ...
                if (readingName) name += newick[pos];
                else lengthStr += newick[pos];
            }
            pos++;
        }
        return { name: name.trim() || undefined, length: parseFloat(lengthStr) || 0 };
    }
    // Remove the final semicolon if it exists
    return parseSubtree(newick.trim().endsWith(';') ? newick.trim().slice(0, -1) : newick.trim());
}
...
function populateControls () {
  ...
  if (treeColour) {
      treeColour.innerHTML = '<option value="--none--">None</option>';
      state.descriptors.forEach(d => treeColour.add(new Option(d, d)));
  }
...
function drawPyramidChart(svg, xDesc, yDesc, xInfo, yInfo) {
  ...
  g.selectAll(".bar-left").data(yCategories).enter().append("rect").attr("class", "bar-left").attr("x", d => xScale(-(counts.get(xCat1)?.get(d) || 0))).attr("y", d => yScale(d)).attr("width", d => xScale(0) - xScale(-(counts.get(xCat1)?.get(d) || 0))).attr("height", yScale.bandwidth()).attr("fill", "#4c78a8");
  g.selectAll(".bar-right").data(yCategories).enter().append("rect").attr("class", "bar-right").attr("x", xScale(0)).attr("y", d => yScale(d)).attr("width", d => xScale(counts.get(xCat2)?.get(d) || 0) - xScale(0)).attr("height", yScale.bandwidth()).attr("fill", "#69b3a2");

  const centerLineX = xScale(0);
  g.append("g").call(d3.axisLeft(yScale).tickSize(0)).attr("transform", `translate(${centerLineX}, 0)`).select(".domain").remove();
...
function drawHeat () {
  ...
  g.append("g").attr("transform", `translate(0,${graphHeight})`).call(d3.axisBottom(x)).selectAll("text").attr("transform", "translate(-10,0)rotate(-45)").style("text-anchor", "end").style("font-size", "14px");
  ...
  x_elements.forEach(pmid => {
    y_elements.forEach(descriptor => {
      ...
      g.append("rect").attr("x", x(pmid)).attr("y", y(descriptor)).attr("width", x.bandwidth()).attr("height", y.bandwidth()).attr("fill", colour(value)).on("mouseover", function () { d3.select(this).attr("stroke", "#000"); }).on("mouseout", function () { d3.select(this).attr("stroke", null); }).append("title").text(`${pmid} – ${descriptor}: ${value ? "present" : "absent"}`);
    });
  });

  const legend = g.append("g").attr("class", "legend").attr("transform", `translate(0, ${graphHeight + 120})`);
  const legendData = [{ value: 1, label: "Present (Data Available)" }, { value: 0, label: "Absent (NA or Missing)" }];
  const legendItems = legend.selectAll(".legend-item").data(legendData).enter().append("g").attr("class", "legend-item").attr("transform", (d, i) => `translate(${i * 250}, 0)`);
  legendItems.append("rect").attr("width", 18).attr("height", 18).attr("fill", d => colour(d.value));
  legendItems.append("text").attr("x", 24).attr("y", 9).text(d => d.label).style("font-size", "16px").attr("alignment-baseline", "middle");
}
  ...
  document.getElementById("searchBox").addEventListener("input", e => {
    ...
    if (tokens.length === 0) {
      ...
      state.sequences = state.allSequences.filter(s => 
        tokens.some(token => s.accession.toUpperCase().includes(token) || (s.pmid && s.pmid.toUpperCase().includes(token)))
      );
    }
...
function loadData () {
  Promise.all([
    d3.csv("sequences.csv"),
    d3.text("RAxML_bestTree.dashboard_test.tree")
  ]).then(([raw, newickString]) => {
    if (!raw.length) throw new Error("CSV empty or not found");
    if (!newickString) throw new Error("Tree file empty or not found");
